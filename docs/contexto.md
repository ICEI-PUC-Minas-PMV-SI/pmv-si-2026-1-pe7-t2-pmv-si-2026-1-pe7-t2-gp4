...
A literatura indica que o absenteísmo resulta de múltiplos fatores que envolvem comportamento do paciente, limitações estruturais e falhas nos processos de comunicação. Estudos baseados em Machine Learning, como Deina (2024) e Salazar et al. (2022), evidenciam que a predição do no‑show é viável e depende do tratamento adequado de dados desbalanceados e da interpretabilidade dos modelos. Pesquisas como a de Valero‑Bover et al. (2022) reforçam que modelos preditivos precisam ser validados no ambiente clínico para que possam ser utilizados de forma segura. Em paralelo, trabalhos descritivos, como Baptista et al. (2023) e Silva et al. (2021), mostram que longos tempos de espera, ausência de lembretes e barreiras socioculturais permanecem como fatores relevantes para as faltas, destacando a necessidade de melhorias nos processos de gestão.

Apesar dessas contribuições, observa-se que a maior parte dos estudos limita-se à identificação de fatores associados ou ao desenvolvimento de modelos de previsão, sem avançar para mecanismos operacionais que utilizem essas informações para otimizar as agendas. Permanece, portanto, uma lacuna entre predição e ação prática. O presente projeto se diferencia ao integrar a estimativa individual de risco com um mecanismo de overbooking seguro, conectando modelagem preditiva e gestão operacional. Essa abordagem responde diretamente a uma necessidade ainda pouco atendida na literatura e tem potencial para melhorar a eficiência das unidades de saúde.
